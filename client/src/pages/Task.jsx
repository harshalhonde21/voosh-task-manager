...
const handlePointerDown = (e, task) => {
  ...
  // Set initial position for touch devices
...
const handlePointerUp = async (e) => {
  ...
  if (draggingElement) {
      const column = document.elementFromPoint(e.clientX, e.clientY).closest('.task-column');
      
      if (column) {
          const taskId = draggingElement.dataset.taskId; // Get task ID from the element
          const newProgress = column.dataset.progress; // Assuming each column has a data-progress attribute
          
          await handleDrop(taskId, newProgress);
      }

      draggingElement.classList.remove('dragging');
      draggingElement.style.opacity = '1'; // Reset opacity
      document.removeEventListener('pointermove', handlePointerMove);
      document.removeEventListener('pointerup', handlePointerUp);
  }
};

// const handleDrop = async (taskId, newProgress) => {
//   try {
//       const token = getAuthToken();  // Retrieve the token for this request
//       await axios.put(`https://voosh-task-manager-f6en.onrender.com/api/v2/tasks/progress-change/${taskId}/progress`, { progress: newProgress }, {
//           headers: {
...
// Attach event listeners to your draggable elements
const draggableElements = document.querySelectorAll('.draggable'); // Adjust selector as needed
draggableElements.forEach(element => {
  element.addEventListener('pointerdown', (e) => handlePointerDown(e, element.dataset.task));
});
